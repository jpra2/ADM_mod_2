...
#        tinv=time.time()
#        LU=linalg.splu(M)
#        ident=np.eye(int(L/2))
#        zeros=np.zeros((int(L/2),int(L/2)),dtype=int)
#        B=np.concatenate([ident,zeros])
#        inversa=csc_matrix(LU.solve(B))
#        B=np.concatenate([zeros,ident])
#        inversa=csc_matrix(hstack([inversa,csc_matrix(LU.solve(B))]))
#        print(inversa.shape[0],inversa.shape[1],"inverteu forma")
#        print(time.time()-tinv,M.shape[0],"tempo de inversão, ordem")
#    return inversa
...
#def lu_inv(M):
#    LU=linalg.splu(M)
#    B=csc_matrix(([1], ([0], [0])), shape=(M.shape[0],1))
#    import pdb; pdb.set_trace()
#    inversa=csc_matrix(linalg.gmres(M,B))
#    for i in range(1,M.shape[0]):
#        B=csc_matrix(([1], ([i], [0])), shape=(M.shape[0],1))
#
#        inversa=vstack([inversa,csc_matrix(linalg.gmres(M,B))])
#    return inversa
...
#    B[0][0]=1
#    inversa=csc_matrix(LU.solve(B)).transpose()
#    for i in range(1,M.shape[0]):
#        B=np.zeros((M.shape[0],1))
#        B[i][0]=1
#        inversa=vstack([inversa,csc_matrix(LU.solve(B)).transpose()])
#    return inversa

#def lu_inv(M):
#    B=np.zeros((M.shape[0],1))
#    B[0][0]=1
#    inversa=csc_matrix(linalg.gmres(M,B)[0])
#    for i in range(1,M.shape[0]):
#        B=np.zeros((M.shape[0],1))
#        B[i][0]=1
#        inversa=vstack([inversa,csc_matrix(linalg.gmres(M,B)[0])])
#    return inversa

#def lu_inv(M):
#
#    tf1=time.time()
#    B=csc_matrix(identity(M.shape[0],dtype=np.int))
#    #B=np.eye(M.shape[0])
#    tb=time.time()-tf1
#    tf2=time.time()
#    inversa=linalg.spsolve(M,B)
#    tinv=time.time()-tf2
#    tf3=time.time()
#    inversa=csc_matrix(inversa)
#    tcinv=time.time()-tf3
#    print(time.time()-tf1,M.shape[0],"tempo de inversão, ordem")
#    return inversa
...
ty=time.time()
print("iniciou____")
nivel_1=M1.mb.get_entities_by_type_and_tag(0, types.MBHEX, np.array([L3_ID_tag]), np.array([1]))
...
if MPFA_NO_NIVEL_2 ==False:
    for i in range(ni):
        lines.append(i)
        cols.append(i)
        data.append(Aie.sum(axis=1)[i]+Aiv.sum(axis=1)[i])
