...
for m2 in meshset_by_L2:
    ...
    for m1 in meshset_by_L1:
        ...
        if ver_1[0] in finos:
            ...
            if max_grad>100*grad_p_res or med1_x<val_barreira or med1_x>val_canal or raz>raz_lim or (max_grad>10*grad_p_res and (r_k_are_ver>40 or k_ares_max/k_vert>100 or k_vert/k_ares_min>100 or k_vert<vt)) or r_k_are_ver>2000:
                aux=1
                tem_poço_no_vizinho=True
...
val1 = 10000
val2 = 4000
v1 = conv.psi_to_Pa(val1)
v2 = conv.psi_to_Pa(val2)
M1.mb.tag_set_data(M1.press_value_tag, volumes_d, np.repeat(v1, len(volumes_d)))
M1.mb.tag_set_data(M1.vazao_value_tag, volumes_n, np.repeat(v2, len(volumes_n)))

values = M1.mb.tag_get_data(M1.press_value_tag, volumes_d, flat=True)
coords = M1.mb.get_coords(M1.all_nodes)
coords = coords.reshape([len(M1.all_nodes), 3])
maxs = coords.max(axis=0)
Lz = maxs[2]
Lz = conv.pe_to_m(maxs[2])

z_elems_d = -1*np.array([conv.pe_to_m(M1.mtu.get_average_position([v])[2]) for v in volumes_d])
delta_z = z_elems_d + Lz
pressao = M1.gama*(delta_z) + values
M1.mb.tag_set_data(M1.press_value_tag, volumes_d, pressao)

values = M1.mb.tag_get_data(M1.vazao_value_tag, volumes_n, flat=True)
z_elems_d = -1*np.array([conv.pe_to_m(M1.mtu.get_average_position([v])[2]) for v in volumes_n])
delta_z = z_elems_d + Lz
pressao = M1.gama*(delta_z) + values
M1.mb.tag_set_data(M1.vazao_value_tag, volumes_n, pressao)
...
nivel_0=rng.unite(nivel_0,nivel_1)

nivel_0v=M1.mb.get_entities_by_type_and_tag(m_vert, types.MBHEX, np.array([L3_ID_tag]), np.array([1]))
nivel_0v=rng.unite(nivel_0v,nivel_1)

ID_global1=M1.mb.tag_get_data(L1_ID_tag,nivel_0, flat=True)
IDs_ADM1=M1.mb.tag_get_data(L2_ID_tag,nivel_0, flat=True)
IDs_AMS1=M1.mb.tag_get_data(fine_to_primal1_classic_tag,nivel_0v, flat=True)
OP3[IDs_AMS1]=csc_matrix((1,OP3.shape[1]))

IDs_ADM_1=M1.mb.tag_get_data(L2_ID_tag,ver, flat=True)
IDs_AMS_1=M1.mb.tag_get_data(fine_to_primal2_classic_tag,ver, flat=True)
lp=IDs_AMS_1
cp=IDs_ADM_1
dp=np.repeat(1,len(lp))
permut=csc_matrix((dp,(lp,cp)),shape=(len(ver),n2))
opad3=OP3*permut
m=find(opad3)
l1=m[0]
c1=m[1]
